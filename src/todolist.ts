...
export default class ToDoList {
  ...
  loadTasks(): void {
    ...
    if (data) {
      this.tasks = JSON.parse(data).map(
        (t: { id: number; name: string; priority: priority; completed: boolean }) =>
  ...
  displayTasks(): void {
    ...
    const createEl = (tag: string, btnClass: string, text = ''): HTMLElement => {
    ...
    tasks.forEach((task) => {
      const li = createEl('li', `task-item priority-${task.getPriority()}`) as HTMLLIElement;
      //checkbox
      ...
      const saveBtn = createEl('button', 'save-btn d-none', 'Save') as HTMLButtonElement;
      //cancel button creating
      const cancelBtn = createEl('button', 'cancel-btn d-none', 'Cancel') as HTMLButtonElement;
      //edit button creation
      const editBtn = createEl('button', 'edit-task', 'Edit') as HTMLButtonElement;
